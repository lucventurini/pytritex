...
def read_10x_molecules(samples: pd.DataFrame, fai: pd.DataFrame, save_dir, client: Client):
    ...
    try:
        ...
        mol = client.submit(dd.merge, f, mol, on="scaffold", how="right")
        mol = client.gather(mol).reset_index(drop=True)
    ...
    barcodes = pd.DataFrame({"barcode_index": np.arange(barcode.shape[0], dtype=np.int32),
                             "barcode": barcode})
    mol = client.submit(dd.merge, barcodes, mol, how="right", on="barcode")
    mol = client.gather(mol).drop("barcode", axis=1).set_index("scaffold_index")
