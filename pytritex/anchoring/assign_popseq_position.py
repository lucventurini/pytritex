...
def assign_popseq_position(cssaln: pd.DataFrame, popseq: pd.DataFrame, anchored_css: pd.DataFrame, wheatchr: pd.DataFrame):
    ...
    popseq_count = popseq_stats.size().to_frame("N")
    ...
    popseq_stats.loc[:, "popseq_Ncss"] = popseq_stats.groupby("scaffold_index")["N"].transform("sum")
    ...
    popseq_stats.loc[:, "popseq_pchr"] = popseq_stats["popseq_Ncss1"].div(popseq_stats["popseq_Ncss"], fill_value=0)
    popseq_stats.loc[:, "popseq_p12"] = popseq_stats["popseq_Ncss2"].div(popseq_stats["popseq_Ncss1"], fill_value=0)
