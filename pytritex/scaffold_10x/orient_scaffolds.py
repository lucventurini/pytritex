...
def _create_association(membership, info, link_pos, client, max_dist_orientation):
    ...
    association = link_pos.set_index("scaffold_index1")
    association = dd.merge(left, association, on="scaffold_index1", how="inner")
    left = m_greater_one[["bin", "super"]].rename(columns={"bin": "bin2", "super": "super2"})
    left.index = left.index.rename("scaffold_index2")
    association = association.reset_index(drop=False).set_index("scaffold_index2")
    ...
    ngroup = association.query("bin2 > bin1").groupby("scaffold_index")
    pgroup = association.query("bin2 < bin1").groupby("scaffold_index")
